...
helpers.formatTime = (pdate) => {
  ...
  let tZone = 'America/Lima',date={},
      ...
      di= new Date(pdate).toISOString(); // timeago
...
/*   di = di.toLocaleString('en-US',{
  ...
  let eu_dddd_str = Convercion_ISO_String(di,'Asia/Oral');
  let fecha_creada = new Date(eu_dddd_str);
...
helpers.timeago_int = (pDate) => {
  return salida = helpers.timeago(pDate.setHours(pDate.getHours()))
}
